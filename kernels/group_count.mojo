...
def _group_count_gpu(
    ...
    if ng <= MAX_GROUPS:
        ...
        @parameter
        def count_kernel_shared(size: Int, ng: Int):
        ...
        ctx.enqueue_function[count_kernel_shared](
            size,
            ng,
            grid_dim=num_blocks,
            block_dim=BLOCK_SIZE,
        ...
        @parameter
        def count_kernel_global(size: Int, ng: Int):
            var block_start = Int(block_idx.x) * BLOCK_SIZE * COARSE_FACTOR
            var tid = block_start + Int(thread_idx.x)
            for c in range(COARSE_FACTOR):
                var i = tid + c * BLOCK_SIZE
        ...
        ctx.enqueue_function[count_kernel_global](
            size,
            ng,
            grid_dim=num_blocks,
            block_dim=BLOCK_SIZE,
