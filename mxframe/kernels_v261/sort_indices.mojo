...
fn _sort_indices_cpu(
    output: ManagedTensorSlice[mut=True, dtype=idx_dtype, rank=1, static_spec=...],
    keys: ManagedTensorSlice[dtype=key_dtype, rank=1, static_spec=...],
    descending_flag: ManagedTensorSlice[dtype=DType.int32, rank=1, static_spec=...],
):
...
fn _merge(
    idx: ManagedTensorSlice[mut=True, dtype=idx_dtype, rank=1, static_spec=...],
    keys: ManagedTensorSlice[dtype=key_dtype, rank=1, static_spec=...],
...
struct SortIndices:
    ...
    fn execute[
        ...
        output: OutputTensor[dtype=idx_dtype, rank=1, static_spec=...],
        keys: InputTensor[dtype=key_dtype, rank=1, static_spec=...],
        descending_flag: InputTensor[dtype=DType.int32, rank=1, static_spec=...],
        ctx: DeviceContextPtr,
    ) raises:
        @parameter
        if target == "cpu":
