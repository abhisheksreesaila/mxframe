...
def bench_scale(N, s=1):
    ...
    li    = make_lineitem(N)
    c3,o3,l3       = make_tpch_q3_tables(15000*s,150000*s,600000*s)
    o12,l12        = make_tpch_q12_tables(50000*s,300000*s)
    p14,l14        = make_tpch_q14_tables(20000*s,200000*s)
    na5,c5,o5,l5   = make_tpch_q5_tables(15000*s,100000*s,400000*s)
    na10,c10,o10,l10 = make_tpch_q10_tables(15000*s,150000*s,600000*s)
    ...
    c13,o13        = make_tpch_q13_tables(150000*s,600000*s)
    p19,l19        = make_tpch_q19_tables(20000*s,200000*s)
    o4,l4          = make_tpch_q4_tables(150000*s,600000*s)
    na9,s9,ps9,p9,o9,l9 = make_tpch_q9_tables(5000*s,2000*s,80000*s,200000*s)
    na11,s11,ps11  = make_tpch_q11_tables()
    c18,o18,l18    = make_tpch_q18_tables(15000*s,60000*s,300000*s)
    p16,s16,ps16   = make_tpch_q16_tables()
    p17,l17        = make_tpch_q17_tables(5000*s,200000*s)
    na2,r2,p2,s2,ps2 = make_tpch_q2_tables()
    s15,l15        = make_tpch_q15_tables(2000*s,200000*s)
    na20,p20,s20,ps20,l20 = make_tpch_q20_tables(5000*s,2000*s,100000*s)
    na21,s21,o21,l21 = make_tpch_q21_tables(2000*s,60000*s,200000*s)
    c22,o22        = make_tpch_q22_tables(150000*s,400000*s)
    print(" done")
    ...
    def row(qname, desc, cpu_fn, gpu_fn, pl_fn, pd_fn):
        ...
        r = {"Q":qname,"Description":desc,
             "MX-CPU(ms)":t(cpu_fn),"MX-GPU(ms)":t(gpu_fn),
    ...
    row("Q15","Top Supplier Rev",lambda:run_q15_mxframe(s15,l15,"cpu"),lambda:run_q15_mxframe(s15,l15,"gpu"),lambda:run_q15_polars(s15,l15),lambda:run_q15_pandas(s15,l15))
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    bench_scale(1_000_000, s=1)
    if args.run10m:
        bench_scale(10_000_000, s=10)
    print("\n\nAll done.")
